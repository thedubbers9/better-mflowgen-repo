...
###########################################################
if {[::legacy::find -design /designs/RightShift_16_flopped] ne ""} {
  puts "** A design with the same name is already loaded. It will be removed. **"
  delete_obj /designs/RightShift_16_flopped
}
...
# Design
###########################################################
read_netlist -top RightShift_16_flopped results_syn/syn_out.v
...
# slack
set _slk_ [::legacy::get_attribute slack /designs/RightShift_16_flopped]
if {[regexp {^-?[0-9.]+$} $_slk_]} {
  set _slk_ [format %.1f $_slk_]
}
if {$_slk_ != "7764.0"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden slack: 7764.0,  current slack: $_slk_"
}
unset _slk_
# multi-mode slack
# tns
set _tns_ [::legacy::get_attribute tns /designs/RightShift_16_flopped]
if {[regexp {^-?[0-9.]+$} $_tns_]} {
...
unset _tns_
# cell area
set _cell_area_ [::legacy::get_attribute cell_area /designs/RightShift_16_flopped]
if {[regexp {^-?[0-9.]+$} $_cell_area_]} {
  set _cell_area_ [format %.0f $_cell_area_]
}
if {$_cell_area_ != "2083"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden cell area: 2083,  current cell area: $_cell_area_"
}
unset _cell_area_
# net area
set _net_area_ [::legacy::get_attribute net_area /designs/RightShift_16_flopped]
if {[regexp {^-?[0-9.]+$} $_net_area_]} {
  set _net_area_ [format %.0f $_net_area_]
}
if {$_net_area_ != "1135"} {
   mesg_send [::legacy::find -message /messages/PHYS/PHYS-92] "golden net area: 1135,  current net area: $_net_area_"
}
unset _net_area_
